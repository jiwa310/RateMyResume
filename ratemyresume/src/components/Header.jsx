    ...
    <header className={`flex justify-between items-center pt-5 md:pt-3 pb-5 md:pb-3 px-6 sticky top-0 w-full font-league-spartan transition-transform duration-200 md:shadow-none shadow-md`} style={{backgroundColor: 'var(--color-background)', zIndex: 999}}>
      ...
      <nav>
        <ul className="flex space-x-5 md:space-x-10">
          <li><button onClick={() => scrollToSection('explore')} className="text-cyan-400 hover:text-cyan-300 transition duration-200 hover:underline ">Explore Resumes</button></li>
          <li><button onClick={() => scrollToSection('upload')} className="text-cyan-400 hover:text-cyan-300 transition duration-200 hover:underline ">Upload Resume</button></li>
        </ul>
      </nav>
    </header>
  )
